...
- `Data/StoreSchema.cs`: ارتقای idempotent تنظیمات فروشگاه و عضویت خبرنامه برای نصب‌های موجود
...
- `src/contexts/`: وضعیت احراز هویت، تنظیمات فروشگاه، سبد و علاقه‌مندی‌ها
...
نسخه فعلی برای شروع سریع از `EnsureCreated` و SQLite استفاده می‌کند. در نخستین اجرا، حساب‌های نمایشی، محصولات، سفارش‌ها و ۲۰ مقاله کامل و تخصصی ایجاد می‌شوند. `BlogSeed.EnsureSchema` و `StoreSchema.Ensure` برای ارتقای نصب‌های SQLite قدیمی، جدول‌های مجله، تنظیمات و خبرنامه را به‌صورت idempotent می‌سازند. Seed مقاله فقط نوشته‌های اولیه مفقود را با تکیه بر `Slug` اضافه می‌کند؛ بنابراین اجرای دوباره برنامه، مقاله تکراری ایجاد نمی‌کند و داده‌های ساخته‌شده توسط مدیر را تغییر نمی‌دهد. تغییرات بعدی schema بهتر است به migration استاندارد EF منتقل شوند.
...
8. routeهای سطح صفحه را lazy نگه دارید تا باندل اولیه بی‌دلیل بزرگ نشود.
