...
نسخه فعلی برای شروع سریع از `EnsureCreated` و SQLite استفاده می‌کند. در نخستین اجرا، حساب‌های نمایشی، محصولات، سفارش‌ها و ۲۰ مقاله کامل و تخصصی ایجاد می‌شوند. `BlogSeed.EnsureSchema` برای ارتقای نصب‌های SQLite قدیمی جدول مجله را به‌صورت idempotent می‌سازد و `BlogSeed.Seed` نیز فقط مقاله‌های اولیه مفقود را با تکیه بر `Slug` اضافه می‌کند؛ بنابراین اجرای دوباره برنامه، مقاله تکراری ایجاد نمی‌کند و نوشته‌های موجود یا ساخته‌شده توسط مدیر را تغییر نمی‌دهد. تغییرات بعدی schema بهتر است به migration استاندارد EF منتقل شوند.
