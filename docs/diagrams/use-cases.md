...
flowchart LR
    ...
    subgraph Store[سامانه مبین سیلور]
        ...
        UC20[عضویت در خبرنامه]
        UC21[لغو سفارش در انتظار]
        UC22[تغییر رمز عبور]
        UC23[تنظیمات فروشگاه و گزارش CSV]
    end
...
- معیار امنیتی: پاسخ نباید وجود یا عدم وجود نام کاربری را افشا کند؛ ششمین تلاش ناموفق برای همان IP و نام کاربری تا ۱۵ دقیقه `429` است.
...
## UC-12: لغو سفارش مشتری

- بازیگر اصلی: مشتری مالک سفارش
- پیش‌شرط: سفارش در وضعیت `در انتظار بررسی` باشد.
- جریان اصلی: مشتری سفارش را از تاریخچه باز می‌کند، لغو را تأیید می‌کند، API مالکیت و وضعیت را کنترل می‌کند، وضعیت `لغو شده` و موجودی اقلام بازگردانده می‌شود.
- استثنا: سفارش کاربر دیگر `404` و سفارش پردازش‌شده `400` است.
- پس‌شرط: سفارش terminal و موجودی قابل فروش اصلاح شده است.

## UC-13: تغییر رمز عبور

- بازیگر اصلی: کاربر احرازشده
- جریان اصلی: رمز فعلی و رمز جدید ارسال می‌شوند؛ API رمز فعلی را verify و برای رمز جدید hash و salt مستقل ایجاد می‌کند.
- استثنا: رمز فعلی اشتباه یا رمز جدید کوتاه با `400` رد می‌شود.

## UC-14: تنظیمات و گزارش مدیریت

- بازیگر اصلی: مدیر
- جریان اصلی: مدیر اطلاعات تماس، اعلان، نام فروشگاه و امکان ثبت سفارش را ذخیره می‌کند؛ داده بلافاصله در پوسته عمومی بازخوانی می‌شود.
- گزارش: مدیر می‌تواند خروجی CSV سفارش‌ها را با داده فعلی دریافت کند.
- معیار امنیتی: تنظیمات کامل فقط برای Admin است و endpoint عمومی شناسه داخلی یا زمان ویرایش را افشا نمی‌کند.

## UC-15: عضویت خبرنامه

- بازیگر اصلی: بازدیدکننده یا مشتری
- جریان اصلی: ایمیل معتبر ثبت و پیام موفقیت قابل‌دسترسی نمایش داده می‌شود.
- جریان جایگزین: ایمیل تکراری بدون ساخت رکورد جدید، پاسخ موفق idempotent می‌گیرد؛ ایمیل نامعتبر `400` است.
