...
- خطای slug تکراری
- نمایش عمومی فقط برای مقاله منتشرشده و موعدرسیده
- چرخه ایجاد پیش‌نویس، ویرایش، انتشار و حذف مقاله با Admin
...
11. جستجو و فیلتر دسته‌بندی مجله
12. صفحه مقاله، فهرست مطالب و مطالب مرتبط
13. ورود مدیر به بخش مقالات و بازشدن ویرایشگر
14. ثبت واقعی خبرنامه و نمایش نتیجه قابل‌دسترسی
15. ثبت‌نام از checkout، ثبت درخواست سفارش و لغو همراه بازگشت موجودی
16. ویرایش محصول، ذخیره تنظیمات و دریافت CSV در پنل مدیر
17. محدودسازی ورود پس از پنج شکست و عدم قفل حساب دیگر
...
- `storefront-v3-desktop.png`
- `storefront-v3-mobile.png`

دو فایل v3 پس از مقایسه کنار کانسپت پذیرفته‌شده در اندازه بومی `1536×1024` و viewport موبایل `390×844` ثبت شده‌اند. snapshot باید فقط پس از بررسی انسانی آگاهانه به‌روزرسانی شود.

## 13. اجرای E2E خودکار

API روی پورت 5088 و Vite روی 5173 باید اجرا باشند، سپس:
...
فایل `playwright.config.ts` به‌طور پیش‌فرض Microsoft Edge را اجرا می‌کند. `E2E_BASE_URL`، `E2E_ADMIN_USER` و `E2E_ADMIN_PASSWORD` برای CI/محیط دیگر قابل override هستند. رمز production را هرگز در فایل تست یا log قرار ندهید.

آخرین رگرسیون کامل در ۱۶ سناریو، `16 passed` گزارش کرده است. پوشش فعلی علاوه بر پوسته فروشگاه، وبلاگ، موبایل، مدیریت، خبرنامه و جریان سفارش، شامل شمارش و deep-link دسته‌ها، session منقضی، تازه‌سازی سبد ذخیره‌شده، تفکیک خطای وبلاگ از empty state، کران تعداد سفارش، محاسبه فروش تأییدشده، ماشین وضعیت سفارش و رد مسیر asset ناامن است. جزئیات در [گزارش کیفیت v4](quality-cycles-v4.md) آمده است.
